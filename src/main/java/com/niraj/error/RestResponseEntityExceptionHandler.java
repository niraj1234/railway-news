...
//				.status(HttpStatus.NOT_FOUND).build();
		...
		message.setMessage("Department Not Found with given id");
...
//				.status(HttpStatus.BAD_REQUEST).build();						
		...
		ErrorMessage message = new ErrorMessage();
		message.setMessage("Department Name is Missing is POST JSON Data ==> "+ errors.toString());
...
//    			.status(HttpStatus.PRECONDITION_FAILED).build();
		...
		ErrorMessage message = new ErrorMessage();
		message.setMessage("Permitted Department Name Not Matched");
		message.setStatus(HttpStatus.PRECONDITION_FAILED);
